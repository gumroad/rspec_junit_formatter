...
private
  ...
  def xml_dump
    xml.instruct!
    xml.testsuite name: "rspec#{ENV['TEST_ENV_NUMBER']}", tests: example_count, failures: failure_count, errors: 0, file: example_group_file_path_for(example), time: "%.6f" % duration, timestamp: started.iso8601 do
